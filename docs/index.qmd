...
```{r}
#| label: paper consumption overview
#| tbl-cap: "xxx"

paper_summarize <- paper |>
  summarise(
    count = n(),
    mean = mean(printing_amount),
    sd = sd(printing_amount),
    median = median(printing_amout),
    min = min(printing_amount),
    max = max(printing_amount)
)
...
ggplot(data = paper, 
       mapping = aes(x = printing_amount, 
                     y = course, 
                     fill = degree)) + 
  geom_col(position = position_dodge()) + 
  labs(title = "i dont know", 
       subtitle = "ysss",
       x = "xsssss", 
       y = "yssssss", 
       fill = "degree", 
